...
class KriteriaPenyakitController extends Controller
{
    ...
    public function hitungKonsistensi()
    {
        ...
        foreach ($kriterias as $col) {
            $jumlah = 0;
            foreach ($kriterias as $row) {
                $jumlah += PerbandinganKriteriaPenyakit::getNilai($row->id, $col->id); // ✅ gunakan model langsung
            }
        ...
        foreach ($kriterias as $row) {
            $total = 0;
            foreach ($kriterias as $col) {
                $nilai = PerbandinganKriteriaPenyakit::getNilai($row->id, $col->id); // ✅ gunakan model langsung
